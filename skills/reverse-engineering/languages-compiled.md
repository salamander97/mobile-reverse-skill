...
.NET 托管程序（C# 编译的 exe/dll）不属于本 mobile distribution 的 bundled workflow。
...
- 脱混淆：ConfuserEx / SmartAssembly / Babel / .NET Reactor → 需使用独立的 .NET 专项资料。
- **IL 优先**：关键判断与 patch 用 IL 编辑器而非 C# 反编译器（避免编译器生成代码重编译失真）
- 红队 Sharp* 工具分析 → 需使用独立的 Windows/.NET 专项资料。

**边界：** IL2CPP / NativeAOT 编译产物是 native（无 CLR），不走 dnSpy → 走 `ida-reverse/` / `radare2/`（见 bundled field notes）。Codegate 2013 两阶段 XOR+AES-CBC 模式见 [tools.md](tools.md#net-analysis) `.NET Analysis` 段。
